/* $Id: setup_factory.hpp 542379 2017-07-31 13:06:45Z dicuccio $
 * ===========================================================================
 *
...
struct NCBI_XBLAST_EXPORT SInternalData : public CObject
{
    ...
    // [SCG] These modifications are to allow multithreading of the BLAST search with the GPU. [04/24/24]
    int * threadStatus; // This should hold the status of the thread. [SCG] [04/24/24]
    int * dbseqsStatus; // This should hold the status of the database sequences. [SCG] [04/24/24]

    BlastSeqSrc ***ptrToSeqsrcArray; // This should hold the pointer to the BlastSeqSrc array. [SCG] [04/24/24]    

    unsigned threadCount; // This should hold the total number of threads that are being used. [SCG] [04/24/24]

    // This should hold the threadIndex so that I can get thread zero to do what you want it to do. [SCG] [04/24/24]
    int threadIndex;
